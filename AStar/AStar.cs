...
namespace AStar
{
    ...
    public class AStar
    {
        ...
        public List<Node> Find(int startX, int startY, int endX, int endY)
        {
            ...
            // 判断目标点是否可移动
            if (_grid.GetWalkable(endX, endY) == false)
            {
                // 寻找目标点附近最近可移动点
                Node node = _grid.FindWalkableNode(startX, startY, endX, endY);
                if (node == null)
                    return null;
                else
                {
            ...
            while (node != _endNode)
            {
                ...
                for (int i = sx; i <= ex; i++)
                {
                    for (int j = sy; j <= ey; j++)
                    {
                        ...
                        if (testNode == node || testNode.walkable == false)
                            continue;
                        g = node.g + 1;
